...
const i18n = {
  fr: {
    ...
    tva: 'TVA', total_ttc: 'Total TTC', logistique_colisage: 'Logistique & colisage camion', btn_devis: 'Générer devis client', btn_colisage: 'Générer fiche colisage', fermer: 'Fermer'
  },
  tr: {
    ...
    tva: 'KDV', total_ttc: 'Toplam TTC', logistique_colisage: 'Kamyon / şantiye malzeme listesi', btn_devis: 'Fransızca müşteri teklifi', btn_colisage: 'Şantiye malzeme fişi', fermer: 'Kapat'
  }
};
...
request.onsuccess = (event) => { db = event.target.result; initialiserParametres(); chargerHistorique(); applyLanguage(); };
...
function calculerCarreleur(e) {
  ...
  const chantier = creerChantierCommun({
    metier: 'Carreleur', client: getInput('client-carrelage'), adresse: getInput('adresse-carrelage'),
    details: `Format : ${format} | Type : ${typeSol}${remisePourcent > 0 ? ` | Remise : ${remisePourcent}%` : ''}`,
...
function calculerPlaquiste(e) {
  ...
  const finance = buildFinance(surface, prixDetail.prixM2, remisePourcent, montantSup, tauxTva, prixDetail.lignes);
  const chantier = creerChantierCommun({
    metier: 'Plaquiste', client: getInput('client-placo'), adresse: getInput('adresse-placo'), details: `${typePlaqueLabel}${remisePourcent > 0 ? ` | Remise : ${remisePourcent}%` : ''}`,
    surface, tauxTva, remisePourcent, labelSup, montantSup, finance, prixDetaille: prixDetail.active, photo: photoCache.placo,
...
function creerChantierCommun({ metier, client, adresse, details, surface, tauxTva, remisePourcent, labelSup, montantSup, finance, colisage, prixDetaille, photo }) {
  return { metier, client: client || 'Client non renseigné', adresse: adresse || '', details, surface,
...
function genererPDFClient(data = null) {
  ...
  ouvrirDocumentImpression({ titre: 'Devis client BATIPIK', contenu: `<div class="head"><div><div class="brand">${safeText(s.artisanNom || 'BÂTIPIK')}</div><div>${safeText(s.artisanContact || '')}</div><div>${safeText(s.artisanSiret || '')}</div></div><div><strong>Devis estimatif client</strong><br>${safeText(chantier.date)}</div></div><div class="box"><h1>${safeText(chantier.metier)} - ${safeText(chantier.client)}</h1><p><strong>Adresse chantier :</strong> ${safeText(chantier.adresse || 'Non renseignée')}</p><p>${safeText(chantier.details)}</p>${remise}</div><h2>Lignes du devis</h2><table>${lignesDevisHtml(chantier)}<tr><td>Total HT</td><td>${safeText(chantier.totalHT)} €</td></tr><tr><td>TVA (${safeText(chantier.tvaLabel)})</td><td>${safeText(chantier.montantTva)} €</td></tr><tr class="total"><td>Total TTC</td><td>${safeText(chantier.totalTTC)} €</td></tr></table><p class="note">Document généré par BATIPIK. Devis estimatif à vérifier selon contraintes réelles du chantier, supports, découpes, accès et conditions de pose. Le colisage matériaux fait l’objet d’une fiche chantier interne séparée.</p><div class="signature"><div>Bon pour accord client</div><div>Signature artisan</div></div>` });
}
function genererFicheColisage(data = null) {
  ...
  ouvrirDocumentImpression({ titre: 'Fiche colisage BATIPIK', styleSupplementaire: '.badge{display:inline-block;background:#d7a93d;color:#17201b;font-weight:bold;border-radius:999px;padding:6px 10px;margin-top:6px}.internal{background:#173d2f;color:white;padding:12px;border-radius:8px;margin:18px 0}.prep td:first-child{width:70%}', contenu: `<div class="head"><div><div class="brand">${safeText(s.artisanNom || 'BÂTIPIK')}</div><div>${safeText(s.artisanContact || '')}</div><div>${safeText(s.artisanSiret || '')}</div></div><div><strong>Fiche chantier interne</strong><br>${safeText(chantier.date)}<br><span class="badge">Colisage</span></div></div><div class="internal"><h1>${safeText(chantier.metier)} - ${safeText(chantier.client)}</h1><p><strong>Adresse :</strong> ${safeText(chantier.adresse || 'Non renseignée')}</p><p>${safeText(chantier.details)}</p><p>Surface chantier : <strong>${safeText(chantier.surface)} m²</strong></p></div><h2>Matériaux à prévoir / commander</h2><table class="prep"><tr><th>Article</th><th>Quantité</th></tr>${colisageRows}</table>${photo}<h2>Notes chantier</h2><table><tr><td style="height:80px;vertical-align:top">Observations, accès, contraintes, fournisseur, couleur, référence produit...</td></tr></table><p class="note">Document interne destiné à l’artisan, au chef d’équipe, à l’apprenti ou au fournisseur. Quantités indicatives à ajuster selon relevé réel, état du support, calepinage, découpes, pertes et conditionnements disponibles.</p>` });
}

function csvEscape(value) { return `"${String(value ?? '').replaceAll('"', '""')}"`; }
function exporterCSV() { if (!db) return; const req = db.transaction([STORE_NAME], 'readonly').objectStore(STORE_NAME).getAll(); req.onsuccess = () => { const rows = [['Date','Client','Adresse','Metier','Surface(m2)','Prix detaille','Total HT(EUR)','TVA','Montant TVA(EUR)','Total TTC(EUR)','Details']]; (req.result || []).forEach((i) => rows.push([i.date,i.client,i.adresse,i.metier,i.surface,i.prixDetaille ? 'Oui' : 'Non',i.totalHT,i.tvaLabel,i.montantTva,i.totalTTC,i.details])); const blob = new Blob(['\uFEFF' + rows.map((r) => r.map(csvEscape).join(';')).join('\n')], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob), link = document.createElement('a'); link.href = url; link.download = 'BATIPIK_Chantiers.csv'; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); }; }
